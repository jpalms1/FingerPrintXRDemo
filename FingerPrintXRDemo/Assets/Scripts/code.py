...
def moveHoxel(X0, Y0, Z0, magF0, shear0, M0):
    if magF0 <= min_force:
        exhaustHoxel0()
    else:
...
def moveFP(X1, Y1, Z1, magF1, shear1):
    if magF1 <= min_force:
        exhaustHoxel1()
    else:
...
while True:
    ...
    if serial.in_waiting > 0:
        ...
        X1 = float(data_list[6])
        Y1 = float(data_list[7])
        Z1 = float(data_list[8])
        magF1 = float(data_list[9])
        shear1 = float(data_list[10])
        M1 = float(data_list[11])

        # Hoxel  as Device 0:
        moveHoxel(X0, Y0, Z0, magF0, shear0, M0)
        # FingerPrint as Device 1:
        # moveFP(X1, Y1, Z1, magF1, shear1, M1)
